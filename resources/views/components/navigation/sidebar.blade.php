...
<div 
  id="sidebar" 
  class="fixed lg:static top-0 left-0 h-full bg-sidebar-background text-sidebar-foreground transition-all duration-500 ease-in-out w-64 flex flex-col shadow-lg overflow-hidden z-40 lg:translate-x-0 -translate-x-full"
>
  <div class="sidebar-header p-4 border-b border-sidebar-border flex items-center justify-between">
    ...
    >
      <x-ui.icon name="chevron-left" class="w-5 h-5 transition-transform duration-500 ease-in-out" id="sidebar-toggle-icon" />
    </button>
  </div>
  ...
  <nav class="flex-1 p-4 space-y-2">
    @php($menuItems = [
      ...
      ['id' => 'members', 'label' => 'Members', 'icon' => 'users', 'route' => 'members.index'],
      ['id' => 'reports', 'label' => 'Laporan', 'icon' => 'bar-chart-3', 'route' => 'reports.index'],
    ...
    @foreach($menuItems as $item)
      ...
      >
        <x-ui.icon name="{{ $item['icon'] }}" class="w-5 h-5 flex-shrink-0 justify-self-center transition-transform duration-500 ease-in-out" />
...
<header class="lg:hidden fixed top-0 left-0 right-0 h-14 bg-card border-b border-border flex items-center justify-between px-4 z-50">
  <button 
    onclick="toggleSidebar()" 
    class="p-2 rounded-md hover:bg-muted/50 transition"
  >
