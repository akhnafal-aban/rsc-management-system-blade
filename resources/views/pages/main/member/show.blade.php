                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        @if($member->status->value === 'ACTIVE')
                            <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-chart-2/20 text-chart-2">Aktif</span>
                        @else
                            <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-destructive/20 text-destructive">Tidak Aktif</span>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-2">
                        @if($member->status->value === 'ACTIVE')
                            <form action="{{ route('member.suspend', $member) }}" method="POST" class="inline-block w-full">
                                @csrf
                                <button type="submit" class="w-full inline-flex items-center justify-center px-3 py-2 border border-destructive text-destructive rounded-lg hover:bg-destructive/10 transition-colors">
                        ...
                        @else
                            <form action="{{ route('member.activate', $member) }}" method="POST" class="inline-block w-full">
                                @csrf
                                <button type="submit" class="w-full inline-flex items-center justify-center px-3 py-2 border border-chart-2 text-chart-2 rounded-lg hover:bg-chart-2/10 transition-colors">
                        ...
                        <form action="{{ route('member.destroy', $member) }}" method="POST" class="inline-block w-full" onsubmit="return confirm('Apakah Anda yakin ingin menghapus member ini? Tindakan ini tidak dapat dibatalkan.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full inline-flex items-center justify-center px-3 py-2 border border-destructive text-destructive rounded-lg hover:bg-destructive/10 transition-colors">
